...
const hospitalIcon = L.divIcon({
  html: `<div style="background:#dc2626;width:42px;height:42px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center"><span style="transform:rotate(45deg);font-size:19px;line-height:36px">🏥</span></div>`,
...
const emergencyHospitalIcon = L.divIcon({
  html: `<div style="background:#991b1b;width:42px;height:42px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center"><span style="transform:rotate(45deg);font-size:19px;line-height:36px">🚨</span></div>`,
  className: "swasth-marker",
  iconSize: [42, 42],
  iconAnchor: [21, 42],
  popupAnchor: [0, -42],
...
const pharmacyIcon = L.divIcon({
  html: `<div style="background:#059669;width:42px;height:42px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center"><span style="transform:rotate(45deg);font-size:19px;line-height:36px">💊</span></div>`,
  className: "swasth-marker",
  iconSize: [42, 42],
  iconAnchor: [21, 42],
  popupAnchor: [0, -42],
...
const appointmentIcon = L.divIcon({
  html: `<div style="background:#7c3aed;width:42px;height:42px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 3px 10px rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center"><span style="transform:rotate(45deg);font-size:19px;line-height:36px">📅</span></div>`,
  className: "swasth-marker",
  iconSize: [42, 42],
  iconAnchor: [21, 42],
  popupAnchor: [0, -42],
...
function FitPlaces({ hospitals, pharmacies, userLocation, appointment }) {
  const map = useMap();
  useEffect(() => {
    const points = [];
...
function FlyTo({ target }) {
  const map = useMap();
  useEffect(() => {
    if (target) map.flyTo(target, 14, { animate: false, duration: 0 });
  }, [target, map]);
...
function directionsUrl(lat, lng, origin) {
...
export default function MedicalMap({
  ...
  const center = userLocation || [22.9734, 78.6569];
  const indiaBounds = [[6.0, 67.0], [37.5, 98.0]];
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {!userLocation && (
        <div className="absolute bottom-4 left-4 z-[2000] max-w-sm bg-white/95 backdrop-blur rounded-xl shadow-xl border border-amber-200 px-3 py-2 pointer-events-none">
